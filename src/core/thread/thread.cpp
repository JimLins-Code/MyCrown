...
namespace crown {
	...
	void Thread::start(ThreadFunction func, void * user_data, u32 stack_size)
	{
...
#if CROWN_PLATFORM_POSIX
		pthread_attr_t attr;
		int err = phtread_attr_init(&attr);
		...
		err = pthread_create(&_priv->handle, &attr, thread_prco, this);
		...
		CE_ASSERT(_priv->handle != NULL, "CreateThread: GelastError = %d", GetLastError());
#endif
